...
class Utils(commands.Cog):
	...
	def suppressed(self,ctx,msg):
		...
		settings = self.client.get_cog("Settings")
	...
	def is_client_admin_only(self,ctx,member=None):
		# Checks only if we're client admin
		settings = self.client.get_cog("Settings")
		if not settings: return False
		member = ctx.author if not member else member
		if not hasattr(member,"roles"): return False # No roles to iterate - can't be client admin
	...
	def is_client_admin(self,ctx,member=None):
		# Checks if the user in the passed context is admin or client admin
		member = ctx.author if not member else member
		return member.permissions_in(ctx.channel).administrator or self.is_client_admin_only(ctx,member)
	...
	async def is_client_admin_only_reply(self,ctx,member=None,message="You do not have sufficient privileges to access this command.",message_when=False):
		# Auto-replies if the user doesn't have admin or client admin privs
		are_we = self.is_client_admin_only(ctx,member)
		if are_we == message_when: await ctx.send(message)
		return are_we

	async def is_client_admin_reply(self,ctx,member=None,message="You do not have sufficient privileges to access this command.",message_when=False):
		# Auto-replies if the user doesn't have admin or client admin privs
		are_we = self.is_client_admin(ctx,member)
		if are_we == message_when: await ctx.send(message)
		return are_we
